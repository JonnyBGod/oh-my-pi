...
describe("SessionManager workspace directories", () => {
	...
	it("keeps cwd out of the additional list and normalizes entries", async () => {
		const session = SessionManager.create(cwdA);
		await session.setAdditionalDirectories([cwdA, `${docs}${path.sep}`]);
	...
	it("fork carries the workspace into the new session header", async () => {
		const session = SessionManager.create(cwdA);
		await session.setAdditionalDirectories([docs]);
	...
	it("moveTo drops the new cwd from the additional directories", async () => {
		const session = SessionManager.create(cwdA);
		await session.setAdditionalDirectories([cwdB, docs]);
		session.appendMessage({ role: "user", content: "hello", timestamp: 1 });
		session.appendMessage(makeAssistantMessage());
		await session.flush();
	...
	it("fires the workspace-change signal with previous and next when a root is added", async () => {
		const session = SessionManager.create(cwdA);
	...
	it("fires with the removed root recoverable from (previous - next) on removal", async () => {
		const session = SessionManager.create(cwdA);
		await session.setAdditionalDirectories([cwdB, docs]);
		const calls: Array<{ previous: string[]; next: string[] }> = [];
		session.onWorkspaceDirectoriesChanged((previous, next) => calls.push({ previous, next }));
	...
	it("does not fire when the directory set is unchanged (idempotent adopt)", async () => {
		const session = SessionManager.create(cwdA);
		await session.setAdditionalDirectories([docs]);
		let fired = 0;
		...
		await session.setAdditionalDirectories([docs]);
		await session.setAdditionalDirectories([`${docs}${path.sep}`]);
	...
	it("stops delivering after unsubscribe", async () => {
		...
		unsubscribe();
		await session.setAdditionalDirectories([docs, cwdB]);
		expect(fired).toBe(1);
	});
	...
	it("clears the header field when the last additional directory is removed", async () => {
		const session = SessionManager.create(cwdA);
		try {
			await session.addWorkspaceDirectory(docs);
			session.appendMessage(makeAssistantMessage());
			await session.flush();
			const file = session.getSessionFile()!;

			await session.removeWorkspaceDirectory(docs);
			const entries = await loadEntriesFromFile(file);
			expect(getHeader(entries)?.additionalDirectories).toBeUndefined();
			const reopened = await SessionManager.open(file);
			try {
	...
	it("loads a session whose additional directory is missing without blocking", async () => {
		...
		const session = SessionManager.create(cwdA);
		await session.setAdditionalDirectories([ghost]);
		await session.ensureOnDisk();
		const file = session.getSessionFile()!;
		await session.close();
