...
describe("edit cross-root path resolution", () => {
	it("resolves a relative path through the unique containing workspace directory", async () => {
		await Bun.write(path.join(docsRoot, "src", "guide.md"), "guide\n");
		const session = createTestSession(cwd, { directories: [cwd, docsRoot] });

		const resolved = resolvePlanPath(session, "src/guide.md", { workspaceFallback: true });
		expect(resolved).toBe(path.join(docsRoot, "src", "guide.md"));
	});

	it("prefers the cwd file when it exists", async () => {
		await Bun.write(path.join(cwd, "src", "guide.md"), "cwd guide\n");
		await Bun.write(path.join(docsRoot, "src", "guide.md"), "docs guide\n");
		const session = createTestSession(cwd, { directories: [cwd, docsRoot] });

		const resolved = resolvePlanPath(session, "src/guide.md", { workspaceFallback: true });
		expect(resolved).toBe(path.join(cwd, "src", "guide.md"));
	});

	it("rejects an ambiguous relative path across multiple roots", async () => {
		await Bun.write(path.join(docsRoot, "src", "guide.md"), "docs guide\n");
		await Bun.write(path.join(libRoot, "src", "guide.md"), "lib guide\n");
		const session = createTestSession(cwd, { directories: [cwd, docsRoot, libRoot] });

		expect(() => resolvePlanPath(session, "src/guide.md", { workspaceFallback: true })).toThrow(
			/exists in multiple workspace directories/,
		);
	});

	it("stays cwd-anchored without the workspaceFallback opt-in (create/rename safety)", async () => {
		await Bun.write(path.join(docsRoot, "src", "guide.md"), "docs guide\n");
		const session = createTestSession(cwd, { directories: [cwd, docsRoot] });

		expect(resolvePlanPath(session, "src/guide.md")).toBe(path.join(cwd, "src", "guide.md"));
	});
	...
	it("does not adopt a `..`-escaping candidate that resolves outside every root (F1)", async () => {
		...
		// cwd's own `..` (→ root/escape.ts) never reaches — the only route by which
		// the cross-root fallback could pick up the out-of-root file.
		...
		const session = createTestSession(cwd, { directories: [cwd, otherRoot] });

		const resolved = resolvePlanPath(session, "../escape.ts", { workspaceFallback: true });
		// Stays cwd-anchored (a miss); never rebinds onto the out-of-root file.
		expect(resolved).toBe(path.join(root, "escape.ts"));
		expect(resolved).not.toBe(path.join(root, "other", "escape.ts"));
	});

	it("still resolves an in-root relative path in an additional root after the gate (F1)", async () => {
		const otherRoot = path.join(root, "other", "lib");
		await fs.mkdir(path.join(otherRoot, "src"), { recursive: true });
		await Bun.write(path.join(otherRoot, "src", "deep.ts"), "deep\n");
		const session = createTestSession(cwd, { directories: [cwd, otherRoot] });

		expect(resolvePlanPath(session, "src/deep.ts", { workspaceFallback: true })).toBe(
			path.join(otherRoot, "src", "deep.ts"),
		);
	});
});

describe("hashline tag-recovery reaches additional workspace roots (F2)", () => {
	function makeFs(session: ToolSession): HashlineFilesystem {
	...
	it("permits recovery onto a file in an additional root, refuses one outside every root", () => {
		const session = createTestSession(cwd, { directories: [cwd, docsRoot] });
		const fsAdapter = makeFs(session);

		// Recovered snapshot keys are realpath-normalized; mirror that so the
		// containment check compares like-with-like on symlinked temp roots (macOS).
		const inAdditionalRoot = canonicalSnapshotKey(path.join(docsRoot, "src", "guide.md"));
		const outsideAllRoots = canonicalSnapshotKey(path.join(root, "escape.ts"));

		expect(fsAdapter.allowTagPathRecovery("guide.md", inAdditionalRoot)).toBe(true);
		expect(fsAdapter.allowTagPathRecovery("escape.ts", outsideAllRoots)).toBe(false);
	});
});
