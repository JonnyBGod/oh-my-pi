...
export class AcpAgent implements Agent {
	...
	async #loadManagedSession(
		...
		if (existing) {
			...
			await this.#applyAdditionalDirectories(existing.session, additionalDirectories);
	...
	async #forkManagedSession(params: ForkSessionRequest): Promise<ManagedSessionRecord> {
		...
		try {
			...
			if (await this.#applyAdditionalDirectories(session, params.additionalDirectories)) {
				await session.sessionManager.ensureOnDisk();
			}
		} catch (error) {
			await this.#disposeStandaloneSession(session);
	...
	async #openStoredSession(
		...
		try {
			...
			if (await this.#applyAdditionalDirectories(session, additionalDirectories)) {
				await session.sessionManager.ensureOnDisk();
			}
		} catch (error) {
			await this.#disposeStandaloneSession(session);
	 ...
	 */
	async #applyAdditionalDirectories(
		session: AgentSession,
		...
		await session.sessionManager.setAdditionalDirectories(additionalDirectories);
		return true;
	}
