...
export async function addWorkspaceDirectory(
	...
	await sessionManager.setAdditionalDirectories([...current.slice(1), resolvedPath]);
...
export async function removeWorkspaceDirectory(
	...
	await sessionManager.setAdditionalDirectories(additional.filter(entry => entry !== resolvedPath));
	await session.refreshBaseSystemPrompt();
